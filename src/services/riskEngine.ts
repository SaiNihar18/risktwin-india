...
export const computeDisasterRisk = (
  ...
  let wildfireFromWeather = clamp01(
    toScore(conditions.temperature, 30, 45) * 0.4 +
      toScore(45 - conditions.humidity, 0, 30) * 0.35 +
      toScore(conditions.windSpeed, 10, 45) * 0.25,
  );

  // Factor in NASA FIRMS active thermal hotspots
  if (conditions.activeFiresNearby && conditions.activeFiresNearby > 0) {
    const fireDensity = Math.min(0.4, conditions.activeFiresNearby * 0.08);
    const fireProximity = conditions.closestFireDistance && conditions.closestFireDistance < 50
      ? 0.35
      : conditions.closestFireDistance && conditions.closestFireDistance < 100
      ? 0.20
      : 0.10;
    wildfireFromWeather = clamp01(wildfireFromWeather * 0.5 + fireDensity + fireProximity);
  }
...
export const computeRiskFactors = (
  ...
  if (conditions.activeFiresNearby && conditions.activeFiresNearby > 0) {
    factors.push({
      ...
      description: `NASA FIRMS satellites detected ${conditions.activeFiresNearby} active thermal fire hotspot(s) within regional radius${conditions.closestFireDistance ? ` (closest ${conditions.closestFireDistance} km away)` : ''}.`,
